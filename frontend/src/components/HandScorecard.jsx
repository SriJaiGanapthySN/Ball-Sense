...
export default function HandScorecard({ context, summary, phase, overs, awardVisible }) {
  ...
  const name = (team) => team === "you" ? "Your XI" : "Computer XI";
  return <section className="broadcast-scorecard" aria-label="Match scorecard">
    ...
    <div className="scorecard-tabs" role="tablist" aria-label="Scorecard innings">
      {summary.sides.map((entry) => <button key={entry.side} id={`score-tab-${entry.side}`} role="tab" aria-controls="score-innings" aria-selected={side === entry.side} onClick={() => setSelected(entry.side)} className={`scorecard-team ${entry.side}`}>
    ...
    <div role="tabpanel" id="score-innings" aria-labelledby={`score-tab-${side}`}>
    ...
    <footer className="scorecard-result" role="status">{context.result ? `${context.result.winner === "tie" ? "Match tied" : context.result.winner === "you" ? "You won" : "Computer won"} / ${context.result.margin}` : chasing ? `${name(side)} need ${needed} from ${remaining} balls${remaining ? ` / Required rate ${(needed * 6 / remaining).toFixed(2)}` : ""}` : phase === "setup" ? "Toss pending" : `${name(context.battingFirst)} bat first`}</footer>
    {awardVisible && <div className="scorecard-award"><Trophy size={30} /><div><span>{summary.award.length > 1 ? "JOINT MEN OF THE MATCH" : "MAN OF THE MATCH"}</span><h3>{summary.award.map((entry) => entry.side === "you" ? "You" : "Computer").join(" & ")}</h3>{summary.award.map((entry) => <p key={entry.side}>{entry.side === "you" ? "You" : "Computer"}: {entry.runs} runs / {entry.wicketsTaken} {entry.wicketsTaken === 1 ? "wicket" : "wickets"} / {entry.points} impact points</p>)}<small>Impact: runs + 25 per wicket. Match winner breaks equal points; tied matches share the award.</small></div></div>}
  </section>;
}